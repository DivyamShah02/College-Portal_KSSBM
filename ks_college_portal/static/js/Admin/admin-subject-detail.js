let csrf_token = null;
let admin_subject_details_url = null;
let admin_subject_announcements_url = null;
let admin_subject_announcements_comments_url = null;
let admin_subject_assignments_url = null;
let admin_subject_attendance_url = null;
let admin_all_assignment_url = null;
let admin_subject_submit_assignments_url = null;
let admin_subject_marked_attendance_url = null;
...
async function HandleSubjectDetail(csrf_token_param, admin_subject_details_url_param, admin_subject_announcements_url_param, admin_subject_announcements_comments_url_param, admin_subject_assignments_url_param, admin_subject_attendance_url_param, admin_all_assignment_url_param, admin_subject_submit_assignments_url_param, admin_subject_marked_attendance_url_param) {
	csrf_token = csrf_token_param;
	admin_subject_details_url = admin_subject_details_url_param;
	admin_subject_announcements_url = admin_subject_announcements_url_param;
	admin_subject_announcements_comments_url = admin_subject_announcements_comments_url_param;
	admin_subject_assignments_url = admin_subject_assignments_url_param;
	admin_subject_attendance_url = admin_subject_attendance_url_param;
	admin_all_assignment_url = admin_all_assignment_url_param;
	admin_subject_submit_assignments_url = admin_subject_submit_assignments_url_param;
	admin_subject_marked_attendance_url = admin_subject_marked_attendance_url_param;
...
async function loadPlacementDetails(subjectId) {
	...
	const url = `${admin_subject_details_url}?` + toQueryString(Params);
                ...
                <div class="card announcement-card mb-3">
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <h5 class="card-title mb-0">${announcement.admin_name}</h5>
...
async function addComment(index, announcement_id) {
	...
	if (comment_content != '') {
		...
		const url = admin_subject_announcements_comments_url;
...
document.getElementById("add_announcement_form").addEventListener("submit", async (event) => {
	...
	const url = admin_subject_announcements_url;
...
document.getElementById("add_assignment_form").addEventListener("submit", async (event) => {
	...
	const url = admin_subject_assignments_url;
	const [success, result] = await callApi("POST", url, formData, csrf_token, true);
...
async function createAttendance() {
	...
	const url = admin_subject_attendance_url;
	const [success, result] = await callApi("POST", url, bodyData, csrf_token);
	if (success) {
		console.log("Result:", result);
...
async function get_submitted_assignment_assignment(assignment_id) {
	...
	const url = `${admin_all_assignment_url}?` + toQueryString(Params);
	const [success, result] = await callApi("GET", url);
	console.log(result);
	if (success) {
		if (result.success) {
...
async function get_submitted_assignment(student_id, assignment_id) {
	...
	const url = `${admin_subject_submit_assignments_url}?` + toQueryString(Params);
	const [success, result] = await callApi("GET", url);
	console.log(result);
	if (success) {
		if (result.success) {
...
async function get_marked_attendance(attendance_id) {
	...
	const url = `${admin_subject_marked_attendance_url}?` + toQueryString(Params);
	const [success, result] = await callApi("GET", url);
	console.log(result);
	if (success) {
		if (result.success) {
